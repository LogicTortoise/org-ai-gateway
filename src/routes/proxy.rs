    ...
    for provider in &order {
        ...
        let outcome = match provider.as_str() {
            ...
            "minimax" | "deepseek" => {
                ...
                //
                // Both MiniMax and DeepSeek serve the Responses API natively
                // at `/v1/responses`, so their Codex-format slot skips the
                // Chat-Completions adapter entirely and goes through
                // `serve_*_responses_passthrough` (transparent pipe). GLM and
                // Kimi still ride the adapter because they have no `/v1/responses`
                // surface to passthrough to.
                if matches!(client_format, CursorFormat::Claude) {
                    serve_native_provider(
                        state.clone(),
                        provider,
                    ...
                    serve_deepseek_responses_passthrough(
                        state,
                        user_id,
                        payload,
                        client_wants_stream,
...
/// adapters (`glm` / `kimi`) — these all share the same shape: a
/// provider-local Responses↔Chat Completions adapter that preserves
...
/// `stream_openai_to_responses_sse` helper. The two providers differ only
...
///
/// MiniMax and DeepSeek are intentionally absent: both serve the Responses
/// API natively at `/v1/responses`, so their Codex slot goes through
/// `serve_minimax_responses_passthrough` / `serve_deepseek_responses_passthrough`
/// (transparent pipe) instead of this adapter. The dispatch site
/// special-cases them.
    ...
    for _ in 0..max_attempts {
        ...
        let send_outcome = match provider {
            ...
            "deepseek" => unreachable!(
                "deepseek is special-cased at the dispatch site and routes to \
                 serve_deepseek_responses_passthrough directly (the upstream \
                 serves the Responses API natively at /v1/responses, so there \
                 is no Chat-Completions adapter to fall through to)"
            ),
...
async fn stream_minimax_responses_passthrough(
    ...
    tokio::spawn(crate::auth::with_request_origin(spawn_origin, async move {
        ...
        // Keep only lifecycle/error telemetry for MiniMax streams. Per-chunk
        // payload previews used to be logged at INFO here, which multiplied a
        // normal response into hundreds of log records and could rapidly fill
        // the gateway disk. Chunk counters remain for error diagnostics.
...
/// Serve a Codex-format request via DeepSeek's **native** Responses API
/// (`/v1/responses`) — a transparent pipe. Mirrors `serve_minimax_responses_passthrough`:
/// DeepSeek officially supports `wire_api = "responses"` from Codex CLI
...
/// so the gateway forwards the payload as-is and pipes / aggregates the
/// upstream Responses SSE stream back to the client. No Responses↔Chat
/// Completions adapter, no `function_call` rewrap, no `type:"custom"` /
/// `type:"namespace"` translator (DeepSeek understands the Responses wire
/// natively).
async fn serve_deepseek_responses_passthrough(
    state: &AppState,
    user_id: &str,
    payload: &Value,
    client_wants_stream: bool,
    ...
    for _ in 0..max_attempts {
        ...
        let selected = {
            ...
            let rate_limits = state.rate_limits.read().await;
            let owner_usage = state.owner_usage.read().await;
            let mut warm = eligible_accounts(&accounts, "deepseek", user_id, &excluded, now, true);
            if owned_only {
                warm.retain(|a| a.owner_user_id == user_id);
            }
            if shared_only {
            ...
            // Only accounts that expose the OpenAI-compatible endpoint can
            // serve this path — same filter as the Chat-Completions adapter
            // and as the MiniMax passthrough.
            warm.retain(deepseek::supports_openai);
            select_account_for_request(&warm, user_id, "deepseek", &rate_limits, &owner_usage)
        };
        let Some(account) = selected else { break };
        selected_any = true;
        excluded.insert(account.id.clone());
        ...
        let send_outcome =
            deepseek::send_deepseek_responses_streaming(&account, &upstream_model, payload).await;

        let upstream = match send_outcome {
            Ok(v) => v,
            Err(err) => {
                apply_account_failure(state, &account.id, ErrorClass::Transient, None, None, false).await;
                last_error = Some((StatusCode::BAD_GATEWAY, json!({ "error": err, "provider": "deepseek" })));
                continue;
            }
        };
        ...
        if !client_wants_stream {
            let body = match crate::util::read_body_capped(upstream, crate::util::max_response_bytes()).await {
                ...
                Err(e) => {
                    ...
                    last_error = Some((
                        StatusCode::BAD_GATEWAY,
                        json!({ "error": format!("failed reading deepseek upstream body: {}", e), "provider": "deepseek" }),
                    ));
                    break;
                }
            };
            ...
            let usage = aggregated
                .as_ref()
                .and_then(|v| v.get("usage"))
                .map(|u| crate::usage::tokens::parse_usage("deepseek", &u.to_string()))
                .unwrap_or_default();
            tracing::warn!(
                "[deepseek_debug] codex_passthrough_body bytes={} aggregated={} preview={:?}",
                body.len(),
                aggregated.is_some(),
                crate::util::truncate_text(&body_str, 500),
            );
            ...
            if !output_has_content {
                ...
                apply_account_failure(state, &account.id, ErrorClass::Transient, None, None, false).await;
                info!(
                    "deepseek_empty_response on {} (usage in={} out={} reason={}); retrying on next account",
                    account.account_label,
                    usage.input_tokens,
                    usage.output_tokens,
                    usage.reasoning_tokens,
                ...
                last_error = Some((StatusCode::BAD_GATEWAY, json!({
                    "error": "deepseek upstream returned a response with no output items (truncated or reasoning-only); try again in 30s",
                    "provider": "deepseek",
                })));
                continue;
            }
            ...
            let resp_json = aggregated.unwrap_or(Value::Null);
            let resp_bytes = serde_json::to_vec(&resp_json)
                .map_err(|e| format!("failed to serialize deepseek Response: {}", e))
                .unwrap_or_default();
            let mut response = (StatusCode::OK, axum::body::Bytes::from(resp_bytes)).into_response();
            let headers = response.headers_mut();
            for (k, v) in passthrough_headers {
        ...
        // the Codex client surfaces the failure and retries (parity with the
        // Chat-Completions SSE translator).
        let response = stream_deepseek_responses_passthrough(
            state.clone(),
            account.clone(),
            user_id.to_string(),
            raw_model.to_string(),
    ...
    match last_error {
        ...
        None => ProviderOutcome::NextProvider(Some(
            (
                StatusCode::SERVICE_UNAVAILABLE,
                Json(json!({ "error": "all deepseek accounts exhausted", "provider": "deepseek" })),
            )
                .into_response(),
        )),
    }
...
/// Pipe a DeepSeek `/v1/responses` SSE stream back to the client verbatim
/// while extracting real usage from the terminal `response.completed` event
/// for the audit row. Empty or errored streams get a synthesised
/// `response.failed` terminal event so the Codex client can retry (mirrors
/// the Chat-Completions SSE translator's behavior).
async fn stream_deepseek_responses_passthrough(
    state: AppState,
    account: UpstreamAccount,
    user_id: String,
    raw_model: String,
    ...
    let account_for_task = account.clone();
    let provider_for_task = "deepseek".to_string();
    let raw_model_for_task = raw_model.clone();
    let user_id_for_task = user_id.clone();
    let spawn_origin = crate::auth::current_origin().unwrap_or_default();
    tokio::spawn(crate::auth::with_request_origin(spawn_origin, async move {
...
/// minimax passthrough's non-2xx branch. Both MiniMax and DeepSeek's OpenAI
/// surfaces return the standard `{"error":{"message":"…","type":"…"}}` shape
/// on non-success. Accepts either a flat `"error": "msg"` (some non-success
/// SSE prelude bodies) or the nested Anthropic-flavored shape; returns `None`
/// when neither form is present so the caller can fall back to a generic
